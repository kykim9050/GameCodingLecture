...
void Town::WeaponUp(Player& _Player)
{
    ...
    //   돈이 부족하면 강화할수 없습니다. 라는 메세지가 뜨게하세요.  V
    //   강화수치마다 강화비용이 증가하게 하세요.
            ...
            int WeaponUpCost = (_Player.Weapon.GetEquipUp() + 1) * 1000;

            if (WeaponUpCost > _Player.GetGold())
            {
                printf_s("강화 비용이 부족합니다.\n");
                int Test = _getch();
                return;
            }

            _Player.SpentGold(WeaponUpCost);
...
void Town::Heal(Player& _Player)
{
    ...
    // 몬스터의 HP가 다시 차게 만드세요.     V
    // FightZone의 가장 처음에 NewMonster.SetHpMax();
    // 로 몬스터의 체력을 다시 100으로 만듬


    // 3. 몬스터가 죽었을때
    //   플레이어가 골드를 얻게 하세요
    //   골드의 범위는 랜덤하게 정하게 하세요   V
    //   100~500 <= 할필요는 없다.
    // FightZone::In에서 몬스터가 죽었을 때, 플레이어의 돈이 100~500사이에 랜덤하게 부여되는 코드 작성
